...
class GPSConverter:
    ...
    async def process_and_send(self):
        ...
        if position:
            ...
            print('Sorry no GPS data was sent over :(')
...
class PIDController:
    ...
    def compute_pid(self, error_x, error_y, dt):
        ...
        try:
            if len(self.x_buffer) >= 11:
                # Apply Savitzky-Golay filter to derivative (1st order)
                dx_series = list(self.x_buffer)
                dy_series = list(self.y_buffer)

                self.derivative_x = savgol_filter(dx_series, window_length=7, polyorder=1, deriv=1, delta=dt)[-1]
                self.derivative_y = savgol_filter(dy_series, window_length=7, polyorder=1, deriv=1, delta=dt)[-1]
            else:
    ...
    async def control_loop(self, tt,database=None):
        ...
        if a==b:
            ...
            print(f"[INFO] Control loop  {(time.time()-start)*1000} ms")
...
class data_base:
    ...
    def append_values(self, elapsed_time, position, error_x, error_y, control_x, control_y, kp, ki, kd, integral_x, integral_y, derivative_x, derivative_y):
        ...
        self.not_filt_x.append(kd*self.pid_controller.non_f_x)
        self.not_filt_y.append(kd*self.pid_controller.non_f_y)
    ...
    def save_data(self):
        ...
        filename = f'trajectory_data_tenth.csv'
        try:
            with open(filename, 'w', newline='') as csvfile:
                ...
                rows = zip(
                    ...
                    self.ki_x, self.ki_y, 
                    self.kd_x, self.kd_y,
                    self.not_filt_x, self.not_filt_y
...
async def main():
    ...
    try:
        ...
        await asyncio.sleep(2) #wait for qtm streaming to start

        mav_handler.connect_to_mavlink()
        await asyncio.sleep(2) # wait for mavlink connection
        ...
        try:
            while time.time() - start_time < 300:
                ...
                await gps_converter.process_and_send() 
                await pid_ctrl.control_loop(start_time,database) 
                await mav_handler.rc_override() 
                time_taken = time.time()-loop_start
                print(f"[INFO] while loop took {time_taken * 1000:.2f} ms")
